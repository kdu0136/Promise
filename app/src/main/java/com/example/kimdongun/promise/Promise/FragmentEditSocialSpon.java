...
public class FragmentEditSocialSpon extends Fragment {
    ...
    public FragmentEditSocialSpon() {
        // Required empty public constructor
    }

    @Override
    public String toString() {
        return "FragmentEditSocialSpon";
    }
            ...
            public void onSuccess(String output) {
                try {
                    ...
                    for (int i = 0; i < friend_obj.length(); i++) {
                        ...
                        for(int j = 0; j < ((EditActivity)getActivity()).friend.size(); j++){
                            AddPromiseSocialListViewItem temp_item = ((EditActivity)getActivity()).friend.get(j);
                ...
                alert_confirm.setMessage("선택한 친구를 등록 하시겠습니까?").setCancelable(false).setPositiveButton("확인",
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ((EditActivity)getActivity()).friend_old.clear();
                                ((EditActivity)getActivity()).friend_old.addAll(((EditActivity)getActivity()).friend);
                                ((EditActivity)getActivity()).friend.clear();
                                ((EditActivity)getActivity()).friend_new.clear();
                                for(int i = 0; i < addPromiseSocialListViewAdapter.getRealCount(); i++){
                                    ...
                                    if(item.isCheck) {
                                        ((EditActivity) getActivity()).friend.add(item); //약속 리스트 갱신
                                    }
                                }
                                for(int i = ((EditActivity)getActivity()).friend_old.size() - 1; i >= 0; i--){ //기존 친구 갱신
                                    AddPromiseSocialListViewItem item_old = ((EditActivity)getActivity()).friend_old.get(i);
                                    boolean exist = false;
                                    for(int j = 0; j < ((EditActivity)getActivity()).friend.size(); j++){
                                        AddPromiseSocialListViewItem item = ((EditActivity)getActivity()).friend_old.get(i);
                                        if(item_old.no == item.no){
                                            exist = true;
                                            break;
                                        }
                                    }
                                    if(!exist){
                                        ((EditActivity)getActivity()).friend_old.remove(i);
                                    }
                                }
                                for(int i = 0; i < ((EditActivity)getActivity()).friend.size(); i++){ //새로운 친구 갱신
                                    AddPromiseSocialListViewItem item = ((EditActivity)getActivity()).friend.get(i);
                                    boolean exist = false;
                                    for(int j = 0; j < ((EditActivity)getActivity()).friend_old.size(); j++) {
                                        AddPromiseSocialListViewItem item_old = ((EditActivity)getActivity()).friend_old.get(i);
                                        if(item_old.no == item.no) {
                                            exist = true;
                                            break;
                                    ...
                                    if(!exist){
                                        ((EditActivity)getActivity()).friend_new.add(item);
                                    }
                                }
